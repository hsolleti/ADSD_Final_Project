    ...
    def setUp(self):
        ...
        if not self.driver:
            ...
            with app.app_context():
                ...
                student1 = User(username='test',password=generate_password_hash('test'), admin=False, student=True, enabled=True)
...
class TestRegisteration(TestBase):
    ...
    def test_invalid_register(self):
        ...
        time.sleep(1)
        user_field = self.driver.find_element_by_id('username-input').send_keys('test')
        password_field = self.driver.find_element_by_id('password-input').send_keys('test')
...
class TestLogin(TestBase):
    ...
    def test_login_by_student(self):
        time.sleep(2)
        user = self.driver.find_element_by_id('username-input').send_keys('test')
        password = self.driver.find_element_by_id('password-input').send_keys('test')
        time.sleep(1)
        button = self.driver.find_element_by_id('log-in-btn').click()
        time.sleep(0.5)
        ...
        self.assertIn(greetings, 'Welcome, test!')
